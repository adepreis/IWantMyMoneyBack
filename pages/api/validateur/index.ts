            ...
            switch (req.body.etat) {
              case "VALIDEE":
                if (await validerNote(req.body.id,note)) {
                  ...
                  res.status(400).json({error : "Vous devez valider toute les lignes pour pouvoir valider la note", code : 400})
                }
                break;
                ...
                case "REFUSEE":
                  if (await refuserNote(req.body.id,note)) {
                    ...
                    res.status(400).json({error : "Vous devez valider ou refuser toute les lignes pour pouvoir refuser la note", code : 400})
                  }
                  break;
              default:
