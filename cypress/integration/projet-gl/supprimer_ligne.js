describe('Supprimer une ligne de frais - Collaborateur', () => {
  ...
  it('Suppression d\'une ligne de frais de remboursement', () => {
    ...
    // Vérification du nombre de "button" APRES suppression d'une ligne
    cy.get('button').should('have.length', 15) // +2/ligneModifiable +1/nouvelleMission +5 -1/ligneSupprimee
    ...
    // Restauration d'une ligne de frais de remboursement
    cy.get('tbody').find('tr').contains('Restaurant').parent().children().last().find('button').last().click()
    // Vérification que le changement a été annulé
    // Changement de mois, en juillet
    cy.get('label').eq(6).click()
    cy.wait(5000)
    // Vérification du nombre de "button" dans le mois de Juillet, prouvant que nous avons bien pu changer de mois
